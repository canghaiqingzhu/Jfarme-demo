...
//删除		
		...
		managerJin.setBounds(35, 42, 93, 23);
		panel_5.add(managerJin);
		
		JButton managerXiao = new JButton("销售管理");
		managerXiao.addActionListener(this);
		managerXiao.setBounds(254, 42, 93, 23);
		panel_5.add(managerXiao);
		
		JButton managerKu = new JButton("库存管理");
		managerKu.addActionListener(this);
		managerKu.setBounds(254, 0, 93, 23);
		panel_5.add(managerKu);
		
		JButton buttonMassage = new JButton("商品录入");
		buttonMassage.addActionListener(this);
		buttonMassage.setBounds(35, 0, 93, 23);
		panel_5.add(buttonMassage);
		
		JButton button_1 = new JButton("条件查询");
		button_1.addActionListener(this);
		button_1.setBounds(151, 20, 93, 23);
		panel_5.add(button_1);
	
		
		JLabel lblJing = new JLabel("New label");
		lblJing.setIcon(new ImageIcon("images/2.JPG"));
		lblJing.setBounds(0, 0,450, 409);
		contentPane.add(lblJing);
	}
	...
	public boolean panRoot(String string){
		boolean s =false;
		Vector data1 = (Vector)data;
		for(int i = 0;i < data.size();i++){
			Vector data2 = (Vector)data1.get(i);
			String w = data2.get(0).toString();
			String v = data2.get(3).toString();
			System.out.println("---panRoot------data2="+data2+"--------------"+i+"----------------");
			if(string.equals(w)){
...
//按钮
	public void actionPerformed(ActionEvent e) {
		if("sure".equals(e.getActionCommand())){
			...
			if(rootru.getText().equals("1") ||rootru.getText().equals("2")||rootru.getText().equals("3")||rootru.getText().equals("4")){
				if(rootru.getText().equals("1")){
				JOptionPane.showMessageDialog(null, "权限限制!!(只能在2~4之间)");
			}else{
				...
				JOptionPane.showMessageDialog(null, "权限限制!!(只能在1~4之间)");
			}
		}else if("delete".equals(e.getActionCommand()))
		{System.out.println("=======delete========");
			boolean t = panRoot(idsan.getText());
			System.out.println("=======t========"+t);
			if(t==true){
				JOptionPane.showMessageDialog(null, "权限限制!!(不可以删除权限为1的账户)");
			}else{
			...
			kf.setVisible(true);
		}else if("商品录入".equals(e.getActionCommand())){
			this.setVisible(false);
			MassageFrame mf = new MassageFrame();
			mf.setVisible(true);
		}else if ("条件查询".equals(e.getActionCommand())) {
			this.setVisible(false);
